...
class UserController extends BaseController
{
    ...
    public function register($error=false)
    {
        ...
        if ($request->isPost() and $error==false) {
    ...
    public function passwordResetConfirmed($request)
    {
        $user = new User();
    ...
    public static function getGITHUBLoginURL()
    {
        $provider=UserController::getGITHUBProvider();
    ...
    public function githubcallback()
    {
        ...
        try {
            ...
            'code' => $_GET['code'],
            'clientId' => 'f78e90dfd11c67e4ced2',
            'clientSecret' => '5112b9a58175ed7667f3d362e1f1bfc2ff94ee43',
            'state' => $_GET['state']
        ]);

            $resourceOwner = $provider->getResourceOwner($accessToken);
            $user = $resourceOwner->toArray();
            $this->oAuth($user['name'], $user['email']);
    ...
    * @return bool
    */
    public function oAuth($username, $mail)
    {
        $user= new User;
        ...
        $error = array("nick" =>_("A User with this nick already exist.") );
        $this->register($error);
    }
