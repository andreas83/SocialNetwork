...
class RemoteContent implements \Iterator
{
    /**
    ...
    public static function fetch($URI)
    {
        ...
        if (302 === $response->getStatus()) {
            return RemoteContent::fetch($response->getRedirectUrl());
        }
    }
    ...
    private static function _parse($HTML)
    {
        ...
        foreach ($tags as $tag) {
            if ($tag->hasAttribute('property') &&
                0 === strpos($tag->getAttribute('property'), 'og:')) {
            ...
            if ($tag->hasAttribute('name') && 'description' === $tag->getAttribute('name')) {
